...
subtitle: All twelve parts of Tony Simons' series, combined, indexed, drilled and annotated into one operator's reference
why: So the whole series can be learned end to end from one offline document instead of twelve separate articles, with every mechanism drilled to the level where a reader can operate it, debug it, and teach it.
date: 2026-09-16
...
```diagram title="The whole Hermes system, one graph"
flowchart TB
...
```diagram title="Stages 1 to 3 · preparing the call"
flowchart LR
...
```diagram title="Stages 4 and 5 · the call, and the loop that repeats"
flowchart LR
...
```diagram title="The three prompt tiers and what invalidates each"
flowchart TB
...
```diagram title="Where sessions survive, and where they do not"
flowchart LR
...
```diagram title="Progressive disclosure, three levels"
flowchart LR
...
```diagram title="Skill lifecycle under the curator"
flowchart LR
...
```diagram title="How the agent reads a skill, section by section"
flowchart LR
...
```diagram title="Why a tool can silently vanish"
flowchart LR
...
```diagram title="The tick · which jobs fire"
flowchart LR
...
```diagram title="Running a due job · two modes"
flowchart LR
...
```diagram title="A three-stage cron pipeline"
flowchart LR
...
```diagram title="A message's path through the gateway"
flowchart LR
  TG["Telegram"] --> AD
  DC["Discord"] --> AD
  SL["Slack"] --> AD
...
```diagram title="Circuit breaker behavior"
flowchart LR
...
```diagram title="Delegate, or script it?"
flowchart TB
...
```diagram title="Hybrid routing keeps your dev server private"
flowchart LR
...
```diagram title="Browser toolset or computer use?"
flowchart TB
...
```diagram title="Read, claim, execute, and how profiles stay out of each other's way"
flowchart TB
...
```diagram title="Four surfaces, one agent loop"
flowchart LR
  A["CLI / TUI"] --> L
...
```diagram title="Context pressure, and the two thresholds that fire"
flowchart LR
...
```diagram title="Should this task go to Hermes at all?"
flowchart TB
...
```file-tree title="The ~/.hermes tree, as the series describes it"
...
```diagram title="What belongs in which file"
flowchart LR
...
```diagram title="Choosing the right extension point"
flowchart TB
...
```code lang=yaml file=~/.hermes/skills/bundles/ship-it.yaml
name: ship-it
...
```diagram title="The shape of a prompt that produces a working system"
flowchart TB
